...
import { BatchEditPosts, BatchPosts, PostEntity } from "@schema/post";
...
export const useGetAllUserPosts = async (status: PostStatus) => {
  return (await api.get(`/posts?status=${status}`)) as AxiosResponse<
    PaginatedResponse<PostEntity>
  >;
};
...
export const useBatchDeletePosts = async (data: BatchPosts) => {
  return await api.delete(`/posts/batch-delete`, {
    data: data,
  });
};

export const useBatchPublishPosts = async (data: BatchPosts) => {
  return await api.patch(`/posts/batch-publish`, data)
}
